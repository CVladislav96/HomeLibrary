...
def create_book():
    try:
        ...
        return jsonify({'message': 'Book created successfully', 'book_id': book.id}), HTTPStatus.CREATED #201
...
def get_all_books():
    try:
        ...
        return jsonify([BookRead.model_validate(book).model_dump() for book in books]), HTTPStatus.OK #200
    except Exception as e:
        return jsonify({'error': str(e)}), HTTPStatus.INTERNAL_SERVER_ERROR #500        
...
def update_book(book_id: int):
    try:
        ...
        if update_book is None:
            return jsonify({'error': 'Book not found'}), HTTPStatus.NOT_FOUND #404
        return jsonify(BookRead.model_validate(update_book).model_dump()), HTTPStatus.OK #200
...
def delete_book(book_id):
    try:
        ...
        return jsonify({'message': 'Book deleted successfully'}), HTTPStatus.NO_CONTENT #204
    except ValueError as e:
        return jsonify({'error': 'Invalid book ID'}), HTTPStatus.BAD_REQUEST #400
    except Exception as e:
        return jsonify({'error': str(e)}), HTTPStatus.INTERNAL_SERVER_ERROR #500 
